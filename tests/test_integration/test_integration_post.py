...
class PostTestCase(TestCase):
    ...
    def test_search_post(self):
        ...
        response = self.client.get(
            reverse("search"), data={"query": "Testing query search"}
        )
        response_no_result = self.client.get(
            reverse("search"),
            data={"query": "ddhjshd" "sjqdgsqhdakzjdbkqdhqqgbj"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["post"][0], post)
        self.assertEqual(len(response.context["post"].object_list), 1)
        self.assertEqual(
            response_no_result.status_code, 200
        )  # catch IndexError
        self.assertEqual(len(
            response_no_result.context["post"].object_list), 0)
