...
class Command(BaseCommand):
    def __init__(self):
        ...
        self.region_dict = {
            ...
            "the democratic republic of congo": None,
            "togo": "Western Africa",
            "tokelau": "Polynesia",
            "tonga": "Polynesia",
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=500000,
            help="Select cities with X minimum of inhabitants | Default : "
            "500000",
        )
        parser.add_argument(
            "--max_population",
            ...
            type=int,
            default=999999999,
            help="Select cities with X maximum of inhabitants",
        )
        parser.add_argument(
            "--keep_db",
            action="store_true",
            help="Add this if you don't want to delete() curent db before "
            "injection",
        )
    ...
    def handle(self, **options):
        ...
        def cities_injection():
            with open(
                "./post/management/commands/worldcities.csv", "r"
            ) as file:
                reader = csv.reader(file)
                min_population = options["min_population"]
                max_population = options["max_population"]
                print(
                    f"Select cities with minimum of {min_population} "
                    "inhabitants and maximum of {max_population}"
                )
                for row in reader:
                    try:
                        if (
                            int(row[9]) >= min_population
                            and int(row[9]) <= max_population
                        ):
                ...
                print(" ")
                return print(
                    f"Database ready : {len(Post.objects.all())} posts !"
                )
